...
export function validateEnvironment(): void {
  ...
  if (jwtAccessSecret && jwtAccessSecret.length < 32) {
    logger.error(
      'JWT_ACCESS_SECRET should be at least 32 characters long for security',
    );
  ...
  if (webhookSecret && webhookSecret.length < 16) {
    logger.error(
      'WEBHOOK_SECRET should be at least 16 characters long for security',
    );
...
export function logEnvironmentInfo(): void {
  ...
  logger.log(
    `Auth Rate Limiting: ${process.env.AUTH_THROTTLE_LIMIT} attempts/5min`,
  );
  logger.log(
    `CORS Origins: ${process.env.CORS_ORIGINS?.split(',').length} configured`,
  );
  ...
  logger.log(
    `JWT secrets configured: ${!!process.env.JWT_ACCESS_SECRET && !!process.env.JWT_REFRESH_SECRET ? '✅' : '❌'}`,
  );
  logger.log(
    `Webhook secret configured: ${!!process.env.WEBHOOK_SECRET ? '✅' : '❌'}`,
  );
}
