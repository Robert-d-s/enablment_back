...
export class TimeService {
  constructor(
    @InjectPinoLogger(TimeService.name) private readonly logger: PinoLogger,
  ...
  async create(
    ...
    try {
      this.logger.debug(
        { startTime, projectId, userId, rateId, totalElapsedTime },
        'Creating time entry',
      );
      ...
      this.logger.info(
        ...
        'Successfully created time entry',
      );
      return createdTime;
    } catch (error) {
      this.logger.error(
        { err: error, startTime, projectId, userId, rateId },
  ...
  async update(
    ...
    try {
      ...
      this.logger.debug(
        { timeId: id, endTime, totalElapsedTime },
        'Updating time entry',
      );
      ...
      this.logger.error(
        { err: error, timeId: id, endTime, totalElapsedTime },
  ...
  async getTotalTimeSpent(
    ...
    try {
      ...
      this.logger.trace(
        'Executing database query to aggregate total time spent...',
      );
      ...
      this.logger.error(
        { err: error, userId, projectId, startDate, endDate },
        'Error calculating total time spent',
      );
      throw new InternalServerErrorException(
  ...
  async getTotalTimeForUserProject(
    ...
    this.logger.debug(
      { userId, projectId },
      'Calculating total time for user project',
    );
    try {
      const aggregatedTime = await this.prisma.time.aggregate({
        where: {
      ...
      this.logger.debug(
        { totalTime },
        'Total time for user project calculated',
      );
      return totalTime;
    } catch (error) {
      this.logger.error(
        { err: error, userId, projectId },
        'Error calculating total time for user project',
      );
      throw new InternalServerErrorException(
